...
// Re-exports for deprecated hooks
// ============================================

/**
 * @deprecated Use useUnifiedAIRouter instead
 * Kept for backward compatibility with useAdvancedRouter imports
 */
export const useAdvancedRouter = () => {
  const router = useUnifiedAIRouter();
  
  return {
    routeTask: async (task: TaskType, options: RouterOptions = {}) => {
    ...
    executeWithFallback: async <T,>(
      ...
      const decision = await router.routeTask(task, options);
      const startTime = Date.now();
      const result = await executeFn(decision.provider, decision.model);
      return { 
        result, 
        provider: decision.provider, 
    ...
    getLoadBalancing: () => router.getLoadBalancing,
    resetMetrics: router.resetMetrics,
  };
};

/**
 * @deprecated Use useUnifiedAIRouter instead
 * Kept for backward compatibility with useSmartAIRouter imports
 */
export const useSmartAIRouter = () => {
  ...
  return {
    executeAI: async (request: {
      ...
      return router.executeAI(
        taskMap[request.task] || request.task as TaskType,
        request.input,
        { preferredProvider: request.preferredProvider, maxCost: request.maxCost }
      );
    },
    loading: router.loading,
    detectWebGPU: router.detectWebGPU,
  };
};
